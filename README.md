...
- **Son delitos reportados, no delitos cometidos.** Cambios en la tasa de denuncia o en la práctica de registro de una agencia se ven como cambios en criminalidad, y no lo son necesariamente. El caso más visible en este dataset: la violación sexual es, de los siete delitos relevados, el único cuyo conteo no cae entre 1990 y 2024 (-0,8%, contra caídas de 41% a 86% en el resto). Es consistente con el consenso de criminología sobre el aumento sostenido de la tasa de denuncia de este delito en las últimas tres décadas, no con un delito que se mantuvo estable mientras todos los demás se desplomaban.
- **El dataset trae un total por condado que duplica, si no se filtra.** Cada condado-año tiene una fila `"County Total"` además de una fila por cada agencia individual que opera ahí. Sumar `Index Total` sobre todas las filas sin excluir el rollup cuenta cada delito dos veces: lo detectamos al calcular estos números (factor de inflación ≈1,49×: 29,5 millones de delitos "ingenuos" vs. 19,8 millones reales para 1990–2024). **Este documento usa exclusivamente las filas `"County Total"` para cualquier agregado por condado, región o estado**, y las filas de agencia individual solo para el ranking de agencias. Las 13 vistas SQL del archivo `vistas_crimes_ny.sql`, tal como están hoy, no aplican este filtro — es la corrección pendiente más importante del proyecto (ver "Qué haría distinto").
...
De los siete tipos de delito relevados, seis cayeron entre 41% y 86% entre 1990 y 2024 (de mayor a menor caída: allanamiento, robo con violencia, robo de vehículos, homicidio, hurto, asalto agravado). La violación sexual es la única excepción: -0,8%, prácticamente plana. Tratándose de uno de los delitos históricamente más subreportados, la lectura más defendible no es que haya dejado de bajar, sino que la tasa de denuncia subió lo suficiente como para compensar cualquier caída real — lo cual es en sí mismo una señal, aunque de un fenómeno distinto al que mide el resto del dataset. *Fuente: `v_ranking_delitos_especificos`, recalculada sobre filas `"County Total"`.*
...
- **Corregir el doble conteo en las 13 vistas SQL.** Es el hallazgo metodológico más importante de este trabajo: las vistas actuales, tal como están escritas, suman las filas `"County Total"` junto con las de cada agencia individual, inflando cualquier agregado por condado o estado en ~1,49×. La corrección es acotada (filtrar por agencia = `"County Total"` en los agregados geográficos, o agregar un flag `is_rollup` a `dim_agencia`) pero no se aplicó todavía sobre los archivos del repositorio ni sobre el dashboard de Power BI, que consume la misma tabla de hechos sin ese filtro.
